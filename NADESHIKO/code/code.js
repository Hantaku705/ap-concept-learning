...
const INSIGHT_CONF = {
  ...
  DEFAULT_TYPE: '単品',
    ...
    // 定時実行の時刻設定
    DAILY_TRIGGER_HOUR: 9  // 毎日の自動実行時刻（0〜23）
  };
  ...
  ui
    ...
    .addSeparator()
    .addItem('毎日9時の自動実行を設定', 'setupDailyTrigger')
  ...
  return getScriptProp_(INSIGHT_CONF.DAILY_SHEET_PROP) || INSIGHT_CONF.SHEET;
  }
  ...
  // ===== 処理順ソート =====
  // SNS優先度: TikTok(0) → YouTube(1) → X(2) → IG(3・最後) → その他(4)
  // ※IG RapidAPIレート制限対策のためIGを最後に処理
  function getSnsPriority_(sns) {
    const priorities = { 'TT': 0, 'YT': 1, 'X': 2, 'IG': 3 };
