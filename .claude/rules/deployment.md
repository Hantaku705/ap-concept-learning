...
本番URLまたはローカルURLをユーザーに共有する前に、以下を確認する：

### チェックリスト

1. **ビルド確認**: `npm run build` が成功しているか
2. **ページアクセス確認**: URLにアクセスしてエラーがないか
   - 404エラーがないか
   - JavaScriptエラーがないか（コンソール確認）
   - APIエラーがないか（Network確認）
3. **主要機能確認**: 重要な機能が動作するか

### 確認方法

| 環境 | 確認方法 |
|------|---------|
| ローカル | `npm run dev` → ブラウザでアクセス |
| 本番 | Playwright E2Eテストまたは手動確認 |
