...
export default function TaglineTable() {
  ...
  const filtered = useMemo(() => {
    ...
    if (axisFilters.size > 0) {
      data = data.filter((d) => {
        ...
        if (axisFilters.has("natural") && d.y > 0) return false
        if (axisFilters.has("glamorous") && d.y <= 0) return false
        return true
      })
    }
    return data
  ...
  const axisOptions: { label: string; value: AxisFilter }[] = [
    { label: "色持ち・機能訴求", value: "functional" },
    { label: "発色・感性訴求", value: "emotional" },
    { label: "ナチュラル", value: "natural" },
    { label: "華やか", value: "glamorous" },
  ]
