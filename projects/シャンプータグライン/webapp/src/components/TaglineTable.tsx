...
type AxisFilter = "functional" | "emotional" | "daily" | "special"
...
const categoryOrder: Record<string, number> = { petit: 0, drugcos: 1, salon: 2 }

const xLabel = (x: number) => x < 0 ? "機能訴求" : "情緒訴求"
const yLabel = (y: number) => y <= 0 ? "日常" : "特別感"
...
export default function TaglineTable() {
  ...
  const filtered = useMemo(() => {
    ...
    if (axisFilters.size > 0) {
      data = data.filter((d) => {
        ...
        if (axisFilters.has("daily") && d.y > 0) return false
        if (axisFilters.has("special") && d.y <= 0) return false
        return true
      })
    }
    return data
  ...
  const axisOptions: { label: string; value: AxisFilter }[] = [
    { label: "機能訴求", value: "functional" },
    { label: "情緒訴求", value: "emotional" },
    { label: "日常", value: "daily" },
    { label: "特別感", value: "special" },
  ]
